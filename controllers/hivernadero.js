...
var controller = {
    ...
    saveData: (req, res) => {
        ...
        var datos_producto = {
            _id: _idproducto,
            nombre: body.nombre,
            instru_cuidado: body.instru_cuidado,
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['hivernadero'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "hivernadero.estado": true }]).
            select({ hivernadero: 1, _id: 1 }).
            sort({ "hivernadero.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "hivernadero._id": req.params._id }, { "hivernadero.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo = Negocio.find({
            ...
            "$or": [
                ...
                { "hivernadero.descripcion": { "$regex": nombrebody, "$options": "i" } }
            ]
        });
        ...
        queryMongo.
            ...
            exec((err, hivernaderos) => {
                ...
                hivernaderos.forEach((hivernadero, index, data) => {

                    hivernadero['hivernadero'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, hivernaderos) => {
                ...
                if (hivernaderos[0].hivernadero.length == 0 || hivernaderos[0].hivernadero.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: hivernaderos[0].hivernadero.length
                });
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            ...
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/hivernadero/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "hivernadero._id": _idProducto },
                { 'hivernadero.imagen._id': _idImage }).
                select({ 'hivernadero.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["hivernadero"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/hivernadero/' + datos['ruta'];
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/hivernadero/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/hivernadero/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/hivernadero';
...
async function eliminarProducto(_id_negocio, _id_producto) {
    ...
    let productUpdate = await Negocio.update({ '_id': _id_negocio },
        {
            $pull: {
                hivernadero: { _id: { $eq: _id_producto } }
            }
        }
    );
...
async function updateStatusProduct(_id_negocio, _id_producto, status) {
    ...
    let productUpdate = await Negocio.updateOne({ '_id': _id_negocio },
        {
            $set: {
                'hivernadero.$[prod].estado': status
            }
        },
        {
...
async function actualizarProducto(_id_negocio, _id_producto, datos_producto) {
    ...
    let productUpdate = await Negocio.updateOne({ '_id': _id_negocio },
        {
            $set: {
                'hivernadero.$[prod]': datos_producto
            }
        },
        {
            arrayFilters: [{ "prod._id": { $eq: _id_producto } }]
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "hivernadero.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "hivernadero.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
