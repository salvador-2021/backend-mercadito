...
var controller = {
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['relojeria'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "relojeria.estado": true }]).
            select({ relojeria: 1, _id: 1 }).
            sort({ "relojeria.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "relojeria._id": req.params._id }, { "relojeria.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, relojerias) => {
                ...
                relojerias.forEach((relojeria, index, data) => {

                    relojeria['relojeria'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, relojerias) => {
                ...
                if (relojerias[0].relojeria.length == 0 || relojerias[0].relojeria.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: relojerias[0].relojeria.length
                });
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            ...
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/relojeria/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "relojeria._id": _idProducto },
                { 'relojeria.imagen._id': _idImage }).
                select({ 'relojeria.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["relojeria"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/relojeria/' + datos['ruta'];
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/relojeria/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/relojeria/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/relojeria';
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "relojeria.$.imagen": { "ruta": rutaImage } }
    });
    return productUpdate;
}
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "relojeria.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
