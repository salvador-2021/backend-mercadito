...
var controller = {
    ...
    saveData: (req, res) => {
        ...
        var result = agregarProduct(_idNegocio, datos_product);
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['cerrajeria'].forEach((producto, index, data) => {
    ...
    getAllProduct: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "cerrajeria.estado": true }]).
            select({ cerrrajeria: 1, _id: 1 }).
            sort({ "cerrajeria.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        ...
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "cerrajeria._id": req.params._id }, { "cerrajeria.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, cerrajerias) => {
                ...
                cerrajerias.forEach((cerrajeria, index, data) => {

                    cerrajeria['cerrajeria'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, cerrajerias) => {
                ...
                if (cerrajerias[0].cerrajeria.length == 0 ||cerrajerias[0].cerrajeria.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: cerrajerias[0].cerrajeria.length
                });
    ...
    updateData: (req, res) => {
        ...
        let resultUpdate = actualizarProduct(_id_negocio, _id_producto, datos_product);
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/cerrajeria/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/cerrajeria/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/cerrajeria';
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            //BORRAR EL ARCHIVO SUBIDO
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/cerrajeria/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "cerrajeria._id": _idProducto },
                { 'cerrajeria.imagen._id': _idImage }).
                select({ 'cerrajeria.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["cerrajeria"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/cerrajeria/' + datos['ruta'];
 ...
 * @param {*} datos_product DATOS DEL PRODUCTO EN JSON
 */
async function agregarProduct(_idNegocio, datos_product) {

    let productInsert = await Negocio.updateOne({ _id: _idNegocio }, {
        $push: {
            'cerrajeria': datos_product
        }
    }, { new: true });
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "cerrajeria.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "cerrajeria.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
