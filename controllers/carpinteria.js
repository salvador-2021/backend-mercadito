...
var controller = {
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['carpinteria'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "carpinteria.estado": true }]).
            select({ carpinteria: 1, _id: 1 }).
            sort({ "carpinteria.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        ...
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "carpinteria._id": req.params._id }, { "carpinteria.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, carpinterias) => {
                ...
                carpinterias.forEach((carpinteria, index, data) => {

                    carpinteria['carpinteria'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, carpinterias) => {
                ...
                if (carpinterias[0].carpinteria.length == 0 || carpinterias[0].carpinteria.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: carpinterias[0].carpinteria.length
                });
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/carpinteria/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/carpinteria/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/carpinteria';
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            //BORRAR EL ARCHIVO SUBIDO
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/carpinteria/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "carpinteria._id": _idProducto },
                { 'carpinteria.imagen._id': _idImage }).
                select({ 'carpinteria.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["carpinteria"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/carpinteria/' + datos['ruta'];
...
async function eliminarProducto(_id_negocio, _id_producto) {
    ...
    let productUpdate = await Negocio.update({ '_id': _id_negocio },
        {
            $pull: {
                carpinteria: { _id: { $eq: _id_producto } }
            }
        }
    );
...
async function updateStatusProduct(_id_negocio, _id_producto, status) {
    ...
    let productUpdate = await Negocio.updateOne({ '_id': _id_negocio },
        {
            $set: {
                'carpinteria.$[prod].estado': status
            }
        },
        {
...
async function actualizarProducto(_id_negocio, _id_producto, datos_producto) {
    ...
    let productUpdate = await Negocio.updateOne({ '_id': _id_negocio },
        {
            $set: {
                'carpinteria.$[prod]': datos_producto
            }
        },
        {
            arrayFilters: [{ "prod._id": { $eq: _id_producto } }]
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "carpinteria.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "carpinteria.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
