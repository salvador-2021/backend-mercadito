...
var controller = {
    ...
    saveData: (req, res) => {
        ...
        var result = agregaProduct(_idNegocio, datos_carne);
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['carniceria'].forEach((producto, index, data) => {
    ...
    getAllProduct: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "carniceria.estado": true }]).
            select({ carniceria: 1, _id: 1 }).
            sort({ "carniceria.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        ...
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "carniceria._id": req.params._id }, { "carniceria.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, carnicerias) => {
                ...
                carnicerias.forEach((carniceria, index, data) => {

                    carniceria['carniceria'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, carnicerias) => {
                ...
                if (carnicerias[0].carniceria.length == 0 || carnicerias[0].carniceria.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: carnicerias[0].carniceria.length
                });
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/carniceria/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/carniceria/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/carniceria';
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            //BORRAR EL ARCHIVO SUBIDO
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/carniceria/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "carniceria._id": _idProducto },
                { 'carniceria.imagen._id': _idImage }).
                select({ 'carniceria.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["carniceria"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/carniceria/' + datos['ruta'];
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "carniceria.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "carniceria.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
