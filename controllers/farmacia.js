...
var controller = {
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['farmacia'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "farmacia.estado": true }]).
            select({ farmacia: 1, _id: 1 }).
            sort({ "farmacia.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        ...
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "farmacia._id": req.params._id }, { "farmacia.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, farmacias) => {
                ...
                farmacias.forEach((farmacia, index, data) => {

                    farmacia['farmacia'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, farmacias) => {
                ...
                if (farmacias[0].farmacia.length == 0 || farmacias[0].farmacia.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: farmacias[0].farmacia.length
                });
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/farmacia/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/farmacia/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/farmacia';
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            //BORRAR EL ARCHIVO SUBIDO
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/farmacia/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "farmacia._id": _idProducto },
                { 'farmacia.imagen._id': _idImage }).
                select({ 'farmacia.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["farmacia"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/farmacia/' + datos['ruta'];
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "farmacia.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "farmacia.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
