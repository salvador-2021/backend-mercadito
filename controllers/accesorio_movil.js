...
var controller = {
    ...
    getAllProductNegocio: (req, res) => {
        ...
        Negocio.find({ _id: _idNegocio }).
            and({ estado: true }).
            select({ accesorio_movil: 1, _id: 1 }).
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['accesorio_movil'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "accesorio_movil.estado": true }]).
            select({ accesorio_movil: 1, _id: 1 }).
            sort({ "accesorio_movil.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        ...
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "accesorio_movil._id": req.params._id }, { "accesorio_movil.$": 1 }).exec((err, listProducto) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            select({ accesorio_movil: 1 }).
            and([{ estado: true }]).
            exec((err, accesorio_moviles) => {
                ...
                if (accesorio_moviles[0].accesorio_movil.length == 0 || accesorio_moviles[0].accesorio_movil.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: accesorio_moviles[0].accesorio_movil.length
                });
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, accesorio_moviles) => {
                ...
                accesorio_moviles.forEach((accesorio_movil, index, data) => {

                    accesorio_movil['accesorio_movil'].forEach((producto, index, data) => {
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/accesorio_movil/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/accesorio_movil/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/accesorio_movil';
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            //BORRAR EL ARCHIVO SUBIDO
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/accesorio_movil/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "accesorio_movil._id": _idProducto },
                { 'accesorio_movil.imagen._id': _idImage }).
                select({ 'accesorio_movil.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["accesorio_movil"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/accesorio_movil/' + datos['ruta'];
...
async function eliminarProducto(_id_negocio, _id_producto) {
    ...
    let productUpdate = await Negocio.update({ '_id': _id_negocio },
        {
            $pull: {
                accesorio_movil: { _id: { $eq: _id_producto } }
            }
        }
    );
...
async function updateStatusProduct(_id_negocio, _id_producto, status) {
    ...
    let productUpdate = await Negocio.updateOne({ '_id': _id_negocio },
        {
            $set: {
                'accesorio_movil.$[prod].estado': status
            }
        },
        {
...
async function actualizarProducto(_id_negocio, _id_producto, datos_producto) {
    ...
    let productUpdate = await Negocio.updateOne({ '_id': _id_negocio },
        {
            $set: {
                'accesorio_movil.$[prod]': datos_producto
            }
        },
        {
            arrayFilters: [{ "prod._id": { $eq: _id_producto } }]
...
async function agregaProducto(_idNegocio, datos_producto) {
    ...
    let productInsert = await Negocio.updateOne({ _id: _idNegocio }, {
        $push: {
            'accesorio_movil': datos_producto
        }
    }, { new: true });
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "accesorio_movil.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "accesorio_movil.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
