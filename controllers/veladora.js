...
var controller = {
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['veladora'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "veladora.estado": true }]).
            select({ veladora: 1, _id: 1 }).
            sort({ "veladora.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "veladora._id": req.params._id }, { "veladora.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, veladoras) => {
                ...
                veladoras.forEach((veladora, index, data) => {

                    veladora['veladora'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, veladoras) => {
                ...
                if (veladoras[0].veladora.length == 0 || veladoras[0].veladora.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: veladoras[0].veladora.length
                });
    ...
    updateDatos: (req, res) => {
        ...
        try {
            ...
            return res.status(500).send({
                status: "error",
                message: "Usuario no identificado" + error
            });
        }
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            ...
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/veladora/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "veladora._id": _idProducto },
                { 'veladora.imagen._id': _idImage }).
                select({ 'veladora.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["veladora"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/veladora/' + datos['ruta'];
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/veladora/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/veladora/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/veladora';
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "veladora.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "veladora.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
