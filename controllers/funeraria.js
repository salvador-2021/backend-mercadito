...
var controller = {
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['funeraria'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "funeraria.estado": true }]).
            select({ funeraria: 1, _id: 1 }).
            sort({ "funeraria.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "funeraria._id": req.params._id }, { "funeraria.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, funerarias) => {
                ...
                funerarias.forEach((funeraria, index, data) => {

                    funeraria['funeraria'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, funerarias) => {
                ...
                if (funerarias[0].funeraria.length == 0 || funerarias[0].funeraria.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: funerarias[0].funeraria.length
                });
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            ...
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/funeraria/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "funeraria._id": _idProducto },
                { 'funeraria.imagen._id': _idImage }).
                select({ 'funeraria.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["funeraria"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/funeraria/' + datos['ruta'];
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/funeraria/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/funeraria/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/funeraria';
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "funeraria.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "funeraria.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
