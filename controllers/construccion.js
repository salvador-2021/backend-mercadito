...
var controller = {
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['construccion'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "construccion.estado": true }]).
            select({ construccion: 1, _id: 1 }).
            sort({ "construccion.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        ...
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "construccion._id": req.params._id }, { "construccion.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, construcciones) => {
                ...
                construcciones.forEach((construccion, index, data) => {

                    construccion['construccion'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, construcciones) => {
                ...
                if (construcciones[0].construccion.length == 0 || construcciones[0].construccion.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: construcciones[0].construccion.length
                });
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/construccion/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/construccion/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/construccion';
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            //BORRAR EL ARCHIVO SUBIDO
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/construccion/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "construccion._id": _idProducto },
                { 'construccion.imagen._id': _idImage }).
                select({ 'construccion.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["construccion"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/construccion/' + datos['ruta'];
...
async function eliminarProducto(_id_negocio, _id_producto) {
    ...
    let productUpdate = await Negocio.update({ '_id': _id_negocio },
        {
            $pull: {
                construccion: { _id: { $eq: _id_producto } }
            }
        }
    );
...
async function updateStatusProduct(_id_negocio, _id_producto, status) {
    ...
    let productUpdate = await Negocio.updateOne({ '_id': _id_negocio },
        {
            $set: {
                'construccion.$[prod].estado': status
            }
        },
        {
...
async function actualizarProducto(_id_negocio, _id_producto, datos_producto) {
    ...
    let productUpdate = await Negocio.updateOne({ '_id': _id_negocio },
        {
            $set: {
                'construccion.$[prod]': datos_producto
            }
        },
        {
            arrayFilters: [{ "prod._id": { $eq: _id_producto } }]
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "construccion.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "construccion.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
