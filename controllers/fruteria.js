...
var controller = {
    ...
    saveData: (req, res) => {
        ...
        var result = agregaProduct(_idNegocio, datos_fruta);
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['fruteria'].forEach((producto, index, data) => {
    ...
    getAllProduct: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "fruteria.estado": true }]).
            select({ fruteria: 1, _id: 1 }).
            sort({ "fruteria.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        ...
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "fruteria._id": req.params._id }, { "fruteria.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, fruterias) => {
                ...
                fruterias.forEach((fruteria, index, data) => {

                    fruteria['fruteria'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, fruterias) => {
                ...
                if (fruterias[0].fruteria.length == 0 || fruterias[0].fruteria.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
                ...
                return res.status(200).send({
                    status: "success",
                    message: fruterias[0].fruteria.length
                });
    ...
    updateData: (req, res) => {
        ...
        let resultUpdate = actualizarProduct(_id_negocio, _id_producto, datos_producto);
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            ...
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/fruteria/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "fruteria._id": _idProducto },
                { 'fruteria.imagen._id': _idImage }).
                select({ 'fruteria.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["fruteria"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/fruteria/' + datos['ruta'];
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/fruteria/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/fruteria/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/fruteria';
...
async function agregarImagen(_idNegocio, _idProducto, rutaImage) {
    ...
    let productUpdate = await Negocio.updateOne({
        ...
        "$push":
            { "fruteria.$.imagen": { "ruta": rutaImage } }
    });
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "fruteria.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
