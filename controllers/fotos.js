...
var controller = {
        ...
        Negocio.find({ _id: _idNegocio }).
            ...
            exec((err, listProducto) => {
                ...
                listProducto.forEach((productos, index, data) => {
                    ...
                    productos['fotos'].forEach((producto, index, data) => {
    ...
    getAllProducto: (req, res) => {
        ...
        Negocio.find().
            and([{ estado: true }, { "fotos.estado": true }]).
            select({ fotos: 1, _id: 1 }).
            sort({ "fotos.precio": 1 }).
            exec((err, listProducto) => {
    ...
    searchproductId: (req, res) => {
        if (req.params._id) { //BUSQUEDAD POR ID
            ...
            Negocio.findOne({ "fotos._id": req.params._id }, { "fotos.$": 1 }).exec((err, listProducto) => {
    ...
    searchproductName: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, fotos) => {
                ...
                fotos.forEach((fotos, index, data) => {

                    fotos['fotos'].forEach((producto, index, data) => {
    ...
    countProductNegocio: (req, res) => {
        ...
        queryMongo.
            ...
            exec((err, fotos) => {
                ...
                if (fotos[0].fotos.length == 0 || fotos[0].fotos.length == null) {
                    return res.status(200).send({
                        status: "success",
                        message: 0
                    });
    ...
    actualizarImage: (req, res) => {
        ...
        if (file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg') {
            ...
            let rutaArchivoEliminar = './uploads/' + _idNegocio + '/fotos/' + file_name;
            ...
            Negocio.findOne(
                { "_id": _idNegocio },
                { "fotos._id": _idProducto },
                { 'fotos.imagen._id': _idImage }).
                select({ 'fotos.imagen': 1 }).
                exec((err, resultquery) => {
                    ...
                    if (resultquery) {
                        ...
                        resultquery["fotos"].forEach((lista_image, index, data) => {
                            ...
                            lista_image["imagen"].forEach((datos, index, data) => {
                                ...
                                if (datos['_id'] == _idImage) {
                                    ...
                                    var rutaArchivo = './uploads/' + _idNegocio + '/fotos/' + datos['ruta'];
    ...
    getImage: (req, res) => {
        ...
        var path_file = './uploads/' + _idNegocio + '/fotos/' + nameImage;
    ...
    deleteImageProduct: (req, res) => {
        ...
        const rutaArchivoEliminar = './uploads/' + _idNegocio + '/fotos/' + _nameImage;
    ...
    deleteAllImageProduct: (req, res) => {
        ...
        const _idnegocio = req.negocio_autentificado._id;
        const rutaArchivoEliminar = '../../uploads/' + _idnegocio + '/fotos';
...
async function actualizarImagen(_idNegocio, _idProducto, _id_image, rutaImage) {
    let productUpdate = await Negocio.updateOne(
        { "_id": { $eq: _idNegocio } },
        { $set: { "fotos.$[prod].imagen.$[img].ruta": rutaImage } },
        {
            arrayFilters: [
